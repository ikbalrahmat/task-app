        ...
        <form method="POST" action="{{ route('subprojects.update', $subproject->id) }}" class="space-y-5">
            ...
            <div>
                ...
                <select name="status" required
                    ...
                    @foreach(\App\Models\Subproject::STATUSES as $s)
                        <option value="{{ $s }}" {{ old('status', $subproject->status) == $s ? 'selected' : '' }}>{{ $s }}</option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="date" name="start_date" value="{{ old('start_date', $subproject->start_date?->format('Y-m-d')) }}"
                ...
                <div>
                    ...
                    <input type="date" name="end_date" value="{{ old('end_date', $subproject->end_date?->format('Y-m-d')) }}"
                           class="w-full bg-white border border-slate-200 text-slate-900 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors">
            ...
            <div class="grid grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="date" name="actual_start_date" value="{{ old('actual_start_date', $subproject->actual_start_date?->format('Y-m-d')) }}"
                           class="w-full bg-white border border-slate-200 text-slate-900 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Realisasi Selesai (Opsional)</label>
                    <input type="date" name="actual_end_date" value="{{ old('actual_end_date', $subproject->actual_end_date?->format('Y-m-d')) }}"
                           class="w-full bg-white border border-slate-200 text-slate-900 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors">
